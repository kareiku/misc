(() => {
    ...
    document
        .querySelectorAll('ytmusic-two-row-item-renderer')
        .forEach((item) => {
            ...
            const url = titleEl
                ? 'https://music.youtube.com/' + titleEl.getAttribute('href')
                : 'Unknown URL';

            const type =
                subtitleEl?.querySelector('span')?.textContent.trim() ||
                'Unknown type';
    ...
    const csv = albums
        .map(
            ({ title, url, type, artist }) =>
                `"${title.replace(/"/g, '""')}","${url}","${type}","${artist.replace(/"/g, '""')}"`
        )
        .join('\n');
